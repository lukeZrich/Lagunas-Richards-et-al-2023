#Master document for creation for bacterial sequence tables.
#Preamble for R 
rm(list = ls())
library(ShortRead)
library(dada2)
...
setwd("~/AnalysisBact007")
#Set working directory for raw reads.
rawDir <- "./Raw"
...
sample.names <- sapply(strsplit(basename(fnFs), "-151118"), `[`, 1)
...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, trimLeft = c(17, 21),
                     maxN=0, truncQ=10, rm.phix=TRUE, truncLen = c(260, 190),
...
track <- cbind(out[,1:2], sapply(dadaFs, getN), sapply(dadaRs, getN), rowSums(seqtab.nochim))
...
#Establish appropriate rarefication level and rarefy
...
#Take the 4 control samples, rarefy to 3800 to encompass all and average reads fro each OTU
...
controls <- rrarefy(controls, 3800)
...
#This happens in fungi data, shouldnt really happen here with bacterial primers
...
#makeblastdb(file = "./AnalysisFungiTax005/Blast/JCVI.Medtr.v4.20130313.fasta", dbtype = "nucl")

length(potential.plant)
hist(nchar(potential.plant))
#Then load library and search (used same library from fungal)
bl <- blast(db = "./AnalysisFungiTax005/Blast/JCVI.Medtr.v4.20130313.fasta")
cl <- predict(bl, potential.plant[8,])
blast.results <- c()
for(i in 1:length(potential.plant)){
  cl <- predict(bl, potential.plant[i, ])
  this.result <- cl[1, ]
  if(is.na(cl[1, ]) == FALSE){
    this.result <- c(this.result, i)
...
potential.plant <- potential.plant[as.numeric(blast.results[, 13])]
my.metric <- (as.numeric(blast.results[, 4]) / nchar(potential.plant)) * (as.numeric(blast.results[, 3]) / 100)
to.dispose <- which(my.metric >= (0.9*0.9) & as.numeric(blast.results[, 3]) >= 90)
...
#Read everything and make rownames uniform
sample.info <- read.csv("SampleInfo.csv")
rownames(sample.info) <- sample.info$SampleID
sample.info <- sample.info[,3:5]
...
#Remove anything that doesn't start with TGGG, TAGG, TCGG, TAAG, TGAG, CAAG, TCGA, TTAG
...
#######01/03/2022 remove all endosphere samples and then remove any reads that are labelled as Ensifer. 
####Then rarefy

#Quantify ensifer reads
e.samples <- sample.info[which(sample.info$FRACTION == "endosphere"), ]
r.samples <- sample.info[which(sample.info$FRACTION == "rhizosphere"), ]
s.samples <- sample.info[which(sample.info$FRACTION == "soil"), ]
e.seqtab <- seqtab.nochim[which(rownames(seqtab.nochim) %in% rownames(e.samples)), ]
r.seqtab <- seqtab.nochim[which(rownames(seqtab.nochim) %in% rownames(r.samples)), ]
s.seqtab <- seqtab.nochim[which(rownames(seqtab.nochim) %in% rownames(s.samples)), ]

NotEnsifer <- rownames(taxa[which(taxa[, "Family"] != "Rhizobiaceae"), ])
Ensifer <- rownames(taxa[which(taxa[, "Family"] == "Rhizobiaceae"), ])

sum(e.seqtab[, which(colnames(e.seqtab) %in% Ensifer)]) / sum(e.seqtab[, which(colnames(e.seqtab) %in% NotEnsifer)])
sum(r.seqtab[, which(colnames(r.seqtab) %in% Ensifer)]) / sum(r.seqtab[, which(colnames(r.seqtab) %in% NotEnsifer)])
sum(s.seqtab[, which(colnames(s.seqtab) %in% Ensifer)]) / sum(s.seqtab[, which(colnames(s.seqtab) %in% NotEnsifer)])


sample.info <- sample.info[which(sample.info$FRACTION != "endosphere"), ]
seqtab.nochim <- seqtab.nochim[which(rownames(seqtab.nochim) %in% rownames(sample.info)), ]
NotEnsifer <- rownames(taxa[which(taxa[, "Genus"] != "Ensifer"), ])
seqtab.nochim <- seqtab.nochim[, which(colnames(seqtab.nochim) %in% NotEnsifer)]

#remove samples with fewer than 1453 reads
totals <- rowSums(seqtab.nochim)
seqtab.nochim <- seqtab.nochim[which(totals >= 1453), ]
sample.info <- sample.info[which(rownames(sample.info) %in% rownames(seqtab.nochim)), ]
#rarefy at 4000 reads
seqtab.nochim <- rrarefy(seqtab.nochim, sample = 1453)
...
#Have to round too; alpha and betadiversity calculations dont like non-integers.
...
#On previously looking there is definitely a problem with rep 1 T
#he alpha diversities are consistently lower in all case and Rep 1 separates from other in PCoA plots
#Remove everything from rep 1
sample.info$Biorep <- substr(rownames(sample.info), 1, 1)
...
for(i in 1:length(rownames(alpha_diversities))){
  alpha_diversities[i, 10:12] <- as.character(sample.info[which(rownames(sample.info) == rownames(alpha_diversities)[i]), 1:3])
}
...
#Shannon diversity
fit <- aov(Shannon ~ TREATEMENT + SOIL.TYPE + FRACTION, data = alpha_diversities)
fiti <- aov(Shannon ~ FRACTION * TREATEMENT * SOIL.TYPE, data = alpha_diversities)
...
psprop <- transform_sample_counts(psrhiz, function(otu) otu/sum(otu))
#ordinate
ord.nmds.bray <- ordinate(ps.prop, method="MDS", distance="bray")
#percentage explained variation for each axis is in
ord.nmds.bray$values[1:2, "Relative_eig"]
